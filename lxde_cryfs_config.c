...
void set_fs_move_index(lxde_cryfs_settings_t* settings, int index) {
	...
	if (config_setting_lookup_string (settings->config, setting_name, &name)) {
		fail = TRUE;
	}
	g_free (setting_name);
	setting_name = g_strdup_printf(LXDE_CRYFS_SETTING_FS_BASE, next);
	if (config_setting_lookup_string (settings->config, setting_name, &basedir)) {
		fail = TRUE;
	}
	g_free (setting_name);
	setting_name = g_strdup_printf(LXDE_CRYFS_SETTING_FS_MOUNT, next);
	if (config_setting_lookup_string (settings->config, setting_name, &mountpoint)) {
		fail = TRUE;
	}
	g_free (setting_name);
	if (!fail) {
...
void lxde_cryfs_add_fs(lxde_cryfs_settings_t* settings, const gchar* basedir, const gchar* mountpoint){
	int max = 0;
	if (config_setting_lookup_int (settings->config, LXDE_CRYFS_SETTING_LAST_INDEX, &max)) {
		return;
	}
...
void lxde_cryfs_remove_fs(lxde_cryfs_settings_t* settings, guint index) {
	int max = 0;
	if (config_setting_lookup_int (settings->config, LXDE_CRYFS_SETTING_LAST_INDEX, &max) || max < 1 || max <= index) {
		return;
	}
	max--;
...
void lxde_cryfs_load_default_settings(lxde_cryfs_settings_t *settings) {
	int num;
	const char *str;
	if (config_setting_lookup_string (settings->config, LXDE_CRYFS_SETTING_DEFAULT_PATH, &str)) {
		config_group_set_string (settings->config, LXDE_CRYFS_SETTING_DEFAULT_PATH, "/");
	}

	if (config_setting_lookup_string (settings->config, LXDE_CRYFS_SETTING_BLOCK_SIZE, &str)) {
		config_group_set_string (settings->config, LXDE_CRYFS_SETTING_BLOCK_SIZE, "32768");
	}

	if (config_setting_lookup_string (settings->config, LXDE_CRYFS_SETTING_UNMOUNT_IDLE, &str)) {
		config_group_set_string (settings->config, LXDE_CRYFS_SETTING_UNMOUNT_IDLE, "");
	}

	if (config_setting_lookup_int (settings->config, LXDE_CRYFS_SETTING_LAST_INDEX, &num)) {
...
const char* lxde_cryfs_get_settings(lxde_cryfs_settings_t *settings, const gchar* setting_name) {
	const char *str = 0;
	if (config_setting_lookup_string (settings->config, setting_name, &str)) {
		// should never happen
		return 0;
	}
	return str;
