...
void basedir_button_click(GtkWidget *widget, gpointer userdata) {
	lxde_cryfs_mount_t* object = (lxde_cryfs_mount_t*) userdata;
	gchar* path = lxde_cryfs_folder_dialog ();
	if (path) {
...
GtkWidget *lxde_cryfs_mount_window() {
	GtkWidget *window;
	...
	g_signal_connect (G_OBJECT (cancel_button), "clicked", G_CALLBACK (close_window), G_OBJECT (window));
